...
export default function NotificationPopup() {
    ...
    socket.on("active-alerts", (message) => {
      if (message.type === 'delta') {
        ...
        setAlerts((prevAlerts) => {
          let newAlerts = prevAlerts.filter(pa => !removed.some(r => r.id === pa.id));
          newAlerts = newAlerts.map(pa => {
            const updatedAlert = updated.find(ua => ua.id === pa.id);
            return updatedAlert || pa;
          });
          newAlerts = [...newAlerts, ...added];
  ...
  <button aria-label="Open notifications" className="p-2 hover:bg-blue-100 hover:text-blue-800 rounded-lg border-gray-200 border" onClick={async () => { await toggle(); notificationsSeen = true; setHasNotifications(false); }}>
        ...
        {unread && hasNotifications && <div className={`absolute -top-3 -right-3 bg-red-500 text-white rounded-full h-6 w-6 animate-ping z-9`}></div>}
      </button>
